/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   print_files_longlist.h                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ylagtab <ylagtab@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/18 09:39:11 by ylagtab           #+#    #+#             */
/*   Updated: 2020/02/19 23:01:09 by ylagtab          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef PRINT_FILES_LONGLIST_H
# define PRINT_FILES_LONGLIST_H

# include "../ft_ls.h"
# include <sys/ioctl.h>
# include <pwd.h>
# include <grp.h>

typedef struct	s_file_details
{
	size_t	size;
	mode_t	st_mode;
	dev_t	st_rdev;
	int		nlink;
	char	perm[10];
	char	acl_ext;
	char	*name;
	char	*user;
	char	*gr;
	char	*time;
}				t_file_details;

typedef struct	s_ll_specs
{
	size_t	total;
	short	hard_links_len;
	short	user_len;
	short	gr_len;
	short	size_len;
	short	time_len;
	short	name_len;
}				t_ll_specs;

void			fill_perm(t_file_details *f_details);
void			fill_acl_ext(t_inode *inode, t_file_details *f_details);
void			fill_nlink(t_file_details *f_details, t_ll_specs *ll_specs,
					nlink_t st_nlink);
void			fill_user(t_file_details *f_details, t_ll_specs *ll_specs,
					uid_t st_uid);
void			fill_group(t_file_details *f_details, t_ll_specs *ll_specs,
					gid_t st_gid);
void			fill_size(t_file_details *f_details, t_ll_specs *ll_specs,
					off_t st_size);
void			fill_time(t_file_details *f_details, t_ll_specs *ll_specs,
					time_t st_time);
void			fill_name(t_file_details *f_details, t_ll_specs *ll_specs,
					t_inode *inode);
void			fill_file_details(t_inode *inode, t_file_details *f_details,
					t_ll_specs *ll_specs);
void			print_file_details(t_file_details *f_details,
					t_ll_specs *ll_specs);
void			print_files_longlist(t_list *files_list, int is_dir);

#endif
